...
2. Mở **toàn bộ** file SQL trong thư mục `supabase/migrations/`, theo đúng thứ tự số, dán vào SQL Editor để thực thi (Run). Chạy thiếu một file là để lại một lỗ hổng hoặc một tính năng hỏng — mỗi file dưới đây nói rõ hậu quả nếu bỏ qua:
   ...
   - `011_conduct_results.sql`: Bảng kết quả rèn luyện từng học kỳ (Thông tư 22 Điều 8). Thiếu file này thì mức giáo viên chủ nhiệm đánh giá nằm lại đúng trên chiếc máy đã gõ nó: học bạ mở ở máy khác trống mục rèn luyện, và danh hiệu không xét được vì danh hiệu cần cả kết quả học tập lẫn kết quả rèn luyện.
   - `012_school_year.sql`: **Bắt buộc — file này chặn một đường mất dữ liệu.** Thêm cột `school_year` (năm học, dạng `2025-2026`) vào bảng điểm, bảng nhận xét và bảng kết quả rèn luyện, rồi dựng lại ràng buộc duy nhất quanh nó. Thiếu file này thì mỗi học sinh chỉ có đúng một bộ điểm cho cả ba năm: hôm giáo viên lớp 11 nhập Toán học kỳ I, dòng Toán học kỳ I hồi lớp 10 bị ghi đè — không báo lỗi, không ghi log, và không có bản sao nào khác. Đến lớp 12, học bạ còn lại một năm trong ba năm đáng lẽ phải có.
   - `013_gradebook_lock.sql`: **Bắt buộc, và phải chạy sau 012.** Bảng `gradebook_locks` (khoá sổ điểm theo từng học kỳ của một năm học) và `assessment_history` (nhật ký thay đổi điểm, chỉ ghi thêm), cùng các trigger thi hành hai việc đó. Thiếu file này thì sổ điểm không bao giờ chốt được — điểm học kỳ I vẫn sửa được vào tháng 4, và dấu hiệu duy nhất là một con số khác với bản in đã ký — còn khi phụ huynh hỏi "điểm con tôi bị sửa", nhà trường chỉ nói được điểm hiện tại là bao nhiêu, không nói được trước đó là bao nhiêu, ai sửa và sửa lúc nào.
3. **Kiểm tra lại sau khi chạy.** Mở `supabase/tests/rls_check.sql` và chạy nó trên chính cơ sở dữ liệu vừa khởi tạo. File này đóng vai từng nhóm người dùng và tự báo lỗi nếu phân quyền sai — trong đó có phép thử "học sinh tự đặt mình thành admin", "giáo viên sửa điểm của học kỳ đã khoá" và "giáo viên xoá dòng nhật ký ghi tên mình". Nó phải chạy hết mà không báo lỗi nào.
