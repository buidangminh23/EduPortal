...
export default function DeadlinesTab({ student }) {
  const { deadlines, addDeadline, toggleDeadlineDone, deleteDeadline, t, language } = useContext(AppContext);
        ...
        <div className="glass-panel">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Calendar size={20} color="var(--accent-primary)" /> {t('Lịch thi & Deadline', 'Exams & Deadlines')}
            </h3>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      <span style={{
                        ...
                        {dl.type === 'exam' ? t('KỲ THI', 'EXAM') : (dl.type === 'event' ? t('SỰ KIỆN', 'EVENT') : t('BÀI TẬP', 'ASSIGNMENT'))}
                      </span>
                      {dl.priority === 'urgent' && <span style={{ fontSize: '0.7rem', padding: '2px 8px', borderRadius: '99px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', fontWeight: 600 }}>{t('GẤP', 'URGENT')}</span>}
                      {dl.type === 'personal' && (
            ...
            {myDeadlines.length === 0 && (
              <div style={{ textAlign: 'center', padding: '30px', color: 'var(--text-muted)' }}>
                ...
                <p>{t('Tuyệt vời! Bạn không có deadline nào sắp tới.', 'Awesome! You have no upcoming deadlines.')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          <div className="glass-panel" style={{ position: 'sticky', top: '90px' }}>
            <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
              <Plus size={20} color="var(--accent-primary)" /> {t('Thêm mục tiêu cá nhân', 'Add Personal Goal')}
            </h3>
            <form onSubmit={handleAddDeadline}>
              <div className="form-group">
                <label className="form-label">{t('TÊN CÔNG VIỆC / MỤC TIÊU', 'TASK / GOAL TITLE')}</label>
                <input
                  type="text"
                  className="form-control"
                  value={newDlTitle}
                  onChange={e => setNewDlTitle(e.target.value)}
                  placeholder={t('VD: Ôn tập 50 từ vựng Tiếng Anh...', 'e.g. Review 50 English vocabulary words...')}
                  required
                />
              </div>
              <div className="form-group">
                <label className="form-label">{t('NGÀY HOÀN THÀNH (DEADLINE)', 'COMPLETION DATE (DEADLINE)')}</label>
                <input
                  type="date"
                  className="form-control"
                  value={newDlDate}
              ...
              <div className="form-group">
                <label className="form-label">{t('MỨC ĐỘ ƯU TIÊN', 'PRIORITY LEVEL')}</label>
                <select
                  className="form-control"
                  value={newDlPriority}
                ...
                >
                  ...
                  <option value="urgent">{t('Gấp, quan trọng', 'Urgent & Important')}</option>
                </select>
              </div>
              <button type="submit" className="btn btn-primary" style={{ width: '100%' }}>
                <CheckCircle size={18} /> {t('Thêm vào danh sách', 'Add to list')}
              </button>
            </form>
            ...
            <div style={{ marginTop: '24px', padding: '16px', background: 'var(--accent-primary-glow)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(99,102,241,0.1)' }}>
              <h4 style={{ fontSize: '0.9rem', color: 'var(--accent-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Sparkles size={16} /> {t('Lời khuyên học tập', 'Study Tip')}
              </h4>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                {t(
                  ...
                  'Breaking large goals down into short-term deadlines helps reduce mental overload and triggers more Dopamine each time you complete a task!'
                )}
              </p>
            </div>
          </div>
