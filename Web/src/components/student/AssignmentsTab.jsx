...
export default function AssignmentsTab({ student }) {
  const { assignments, submissions: allSubmissions, submitAssignment, t } = useContext(AppContext);
  ...
  const handleSubSubmit = (assignmentId) => {
    ...
    if (!text || !text.trim()) {
      alert(t('Vui lòng nhập nội dung câu trả lời/bài làm trước khi nộp!', 'Please enter your answer/submission content before submitting!'));
      return;
    }
    submitAssignment(assignmentId, student.id, text);
    setSubmissionTexts(prev => ({ ...prev, [assignmentId]: '' }));
    alert(t('Đã nộp bài làm thành công! Hệ thống sẽ cập nhật trạng thái chấm điểm của giáo viên bộ môn.', 'Submission successful! The system will update your grade status once reviewed by your teacher.'));
  };

  const handleQuizSubmit = (assignment, questions) => {
    const answers = quizAnswers[assignment.id] || {};
    if (Object.keys(answers).length < questions.length) {
      alert(t('Vui lòng trả lời đầy đủ tất cả các câu hỏi trắc nghiệm trước khi nộp!', 'Please answer all multiple-choice questions before submitting!'));
      return;
    }
    let correctCount = 0;
    ...
    alert(t(`Đã nộp bài trắc nghiệm thành công! Hệ thống AI đã tự động chấm điểm bài làm của bạn: ${finalScore}/10 điểm.`, `Quiz submitted successfully! AI auto-graded your submission: ${finalScore}/10 points.`));
  };
  ...
  return (
    <div className="animate-fade">
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ fontSize: '1.25rem', marginBottom: '6px' }}>{t('Danh sách Bài Tập Về Nhà được giao', 'Assigned Homework & Tasks')}</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          {t('Hãy hoàn thành đúng hạn các bài tập tự luận/trắc nghiệm từ giáo viên bộ môn.', 'Complete essay and multiple-choice assignments from subject teachers on time.')}
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
        {myAssignments.length === 0 ? (
          <div className="glass-panel" style={{ gridColumn: '1/-1', textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
            {t(`Chưa có bài tập nào được giao cho lớp ${student.class}.`, `No assignments assigned to class ${student.class}.`)}
          </div>
        ) : (
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <span className="badge badge-info" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      {isQuiz && <Sparkles size={12} />} {t(assignment.subject)} • {assignment.teacherName}
                    </span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{t('Hạn: ', 'Due: ')}{assignment.deadline}</span>
                  </div>
                  ...
                  {assignment.fileName && (
                    <div style={{ 
                      ...
                      <Paperclip size={14} />
                      <span>{t('Tệp đính kèm: ', 'Attachment: ')}<strong style={{ textDecoration: 'underline', cursor: 'pointer' }}>{assignment.fileName}</strong></span>
                    </div>
                  )}
                  ...
                  {isGraded ? (
                    <div style={{ marginTop: '10px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', borderRadius: '8px', background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.15)', marginBottom: '8px' }}>
                        ...
                        <div style={{ fontSize: '0.8rem' }}>
                          <div style={{ fontWeight: 700, color: 'var(--accent-secondary)' }}>{t('Đã chấm điểm (thang 10)', 'Graded (scale 10)')}</div>
                          <div style={{ color: 'var(--text-muted)' }}>{t('Bài nộp hoàn thành thành công', 'Submission successfully completed')}</div>
                        </div>
                      </div>
                      {feedback && (
                        <p style={{ fontSize: '0.85rem', padding: '8px 12px', background: 'rgba(0, 0, 0, 0.01)', borderLeft: '3px solid var(--accent-secondary)', borderRadius: '0 8px 8px 0', fontStyle: 'italic', margin: '4px 0', color: 'var(--text-secondary)' }}>
                          <strong>{t('Lời phê:', 'Teacher Feedback:')}</strong> "{feedback}"
                        </p>
                      )}
                      {sub.submissionText && (
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: 8, padding: 8, background: '#f8fafc', borderRadius: 6, border: '1px solid #e2e8f0' }}>
                          <strong>{t('Chi tiết bài làm:', 'Submission details:')}</strong> {sub.submissionText}
                        </div>
                      )}
                    </div>
                  ) : isSubmitted ? (
                    <div style={{ padding: '10px 14px', borderRadius: '8px', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.15)', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 600, color: 'var(--accent-primary)', marginBottom: '4px' }}>
                        <span>{t('Đã nộp bài làm', 'Submitted')}</span>
                        <span>{sub.submittedAt}</span>
                      </div>
                      <span style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>{t('Đang chờ thầy/cô xem xét chấm điểm...', 'Awaiting teacher grading...')}</span>
                    </div>
                  ) : isQuiz ? (
                    ...
                    <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.85rem', color: '#6366f1', fontWeight: 600 }}>
                        <Sparkles size={14} />
                        <span>{t('Bài kiểm tra trắc nghiệm tự động bằng AI', 'AI Automated Quiz Test')}</span>
                      </div>
                      
                      {assignment.questions.map((q, idx) => (
                        <div key={idx} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: 12 }}>
                          <div style={{ fontSize: '0.88rem', fontWeight: 700, color: '#1e293b', marginBottom: 8 }}>{t(`Câu ${idx + 1}: `, `Question ${idx + 1}: `)}{q.question}</div>
                      ...
                      >
                        <CheckSquare size={16} /> {t('Nộp bài trắc nghiệm AI', 'Submit AI Quiz')}
                      </button>
                    </div>
                  ) : (
                    ...
                    <div style={{ marginTop: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', color: 'var(--accent-danger)', fontWeight: 600, marginBottom: '6px' }}>
                        <span>🔴 {t('Chưa nộp bài làm', 'Not Submitted')}</span>
                      </div>
                      <textarea
                        className="form-control"
                        rows="3"
                        placeholder={t('Nhập nội dung bài giải hoặc văn bản nộp bài...', 'Enter your solution or submission response...')}
                        value={subText}
                      ...
                      >
                        {t('Nộp bài tập trực tuyến', 'Submit Homework Online')}
                      </button>
                    </div>
                  )}
                </div>
